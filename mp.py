...
simple-to-implement locking mechanism that allows you to
synchronize threads.Threading in python is used to run
multiple threads (tasks, function calls) at the same time."""

"""The OS module in Python provides a way of using
operating system dependent functionality. The functions
that the OS module provides allows you to interface with
the underlying operating system that Python is running
on – be that Windows, Mac or Linux."""

"""ttkthemes includes a wide variety of different themes,
and there is always room for more themes, no matter how
ugly or obscure! Even though some themes may not be used in practice,
the original goal of the project has not been
forgotten: To gather and preserve all themes.
"""
...
such as objects, numbers, and strings. It also provides functionality
...
statusbar = ttk.Label(root, text="Welcome to PICTO Music Player",
...
def about_us():
    tkinter.messagebox.showinfo('About PICTO Music Player',
                                '''This is a music player developed using Python Tkinter and mutagen.mp3 module built by
\n\t            Sudarsun.S''')
